...
The second pitfall is **incorrect pool sizing and missing rejection handling**. The builder app's integrity score shows this clearly: if walls divide the hull into grossly unequal sections, the largest section becomes the weak point. Translated to software: an oversized bulkhead pool (e.g., 200 threads for a service that needs 20) wastes resources and can mask latency issues, while an undersized pool rejects legitimate traffic prematurely. Worse, many implementations accept the bulkhead pattern but never wire up the rejection path — when the pool is full, the request blocks indefinitely instead of failing fast, converting a bulkhead into a deadlock. The simulation's explicit `REJECTED — bulkhead full` log and counter are not just cosmetic; they model the mandatory fast-fail behavior. Every bulkhead must have a defined rejection policy (fail-fast, queue-with-timeout, or fallback), and that policy must be tested under load.

The third pitfall is **missing observability on compartment state**, which causes silent degradation. The simulator tracks breach count vs. sealed count per frame; the dashboard shows per-pool utilization percentage with an 85% degradation threshold. Without these metrics, a half-failed bulkhead looks identical to a healthy system from outside. In production, teams deploy bulkheads but fail to alert on rejection rate, pool saturation, or queue depth — so when three of five pools are saturated, the only signal is a vague latency increase. The degradation-class visual flip (green → red at 85%) models the alerting threshold that production bulkheads require: per-pool utilization and rejection rate must be instrumented, dashboarded, and alerted on, or the isolation guarantee is unverifiable.
